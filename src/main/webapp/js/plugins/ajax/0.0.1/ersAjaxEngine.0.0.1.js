		...
		success : function(html) {
			if(ajRequestData.additionResultsParams != null){
				if(ajRequestData.additionResultsParams.arg1 != null){
					ajRequestData.resultsFunc.call(this,
												   html,
		...
		complete : function() {
			if(ajRequestData.chain != null){
				for(var i = 0; i<ajRequestData.chain.length; i++){
					if(ajRequestData.chainArgs && ajRequestData.chainArgs.length > 0){
						ajRequestData.chain[i].call(this,ajRequestData.chainArgs[0]);
					}else{
...
function get_main_section_div() {
   ...
   for(var i = 0; i< document.getElementsByTagName("script").length; i++){
	   if(document.getElementsByTagName("script")[i].src.match(/\w+=\w+/g)){
		   qs = document.getElementsByTagName("script")[i].src.match(/\w+=\w+/g), qstring = {},t,i = qs.length;
		   process = true;
	   }
	   if(process){
			while (i--) {
...
function _doServerCommunications( action, aResultsFunc, formId, showWait, waitNode, hideNode) {
	var commRequest = {ajaxHandler: action, 
...
function _commonSaveAlertMessageTrigger(){
	  $('#ex6b').jqm({
		  ajax: context_get_main_section_div()+'/access/save_confirmation_message_1',
